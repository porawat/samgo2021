...
router.post('/mobile50tv', async (req, res) => {
  res.setHeader("Content-Type", "application/json");
 // var setyear = req.body.year;
  var empCode = req.body.empCode;
  var sql = `SELECT * FROM  SAMCOSP.dbo.v50tv WHERE (EmpID = '${empCode}')`;

  pay.query(sql, (err, rs) => {
    if (err) res.status(200).json({ results: 'error' });
    if (rs.rowsAffected[0] === 0) {
      res.json({ results: 'error' })
      //console.log(rs.rowsAffected[0])
    } else {
      res.json({ results: `success`, rows: rs.recordset })
    }
  })
})

router.get('/mobile50tv', async (req, res) => {
  var sql = `SELECT * FROM  SAMCOSP.dbo.v50tv WHERE (EmpID = '541351')`;
  pay.query(sql, (err, rs) => {
...
router.post('/mobileoffgrid', (req, res) => {
  res.setHeader("Content-Type", "application/json");
  //var month = JSON.parse(req.body.month);
  var month = req.body.DateOff;
  var year = req.body.year;
  var empCode = req.body.code;
  var arr = [];
  var sql = `SELECT  *
  FROM            SAMCOSP.dbo.V_dropoffold
  WHERE        (code = '${empCode}') AND (DateOff = '${month}') AND (offyear = '${year}')`
  pay.query(sql, (err, rs) => {
    if (err) res.json({ results: `error`, rows: sql });
    if (rs.rowsAffected[0] === 0) {
      res.json({ results: `error` })
...
router.get('/dropoff', async (req, res) => {
  ...
  pay.query(sql, (err, records) => {
    res.send(records.recordset)
  })
});
router.get('/appvsersion', async (req, res) => {
  res.setHeader("Content-Type", "application/json");
  var sql = `SELECT  appName, appVersion, appNumber, appNote, appUpdate,appUrl FROM  SAMCOSP.dbo.DropoffVersion`;
  pay.query(sql, (err, records) => {
    // console.log(records.recordset[0])
    res.json({ results: `success`, rows: records.recordset })
    //res.json({ results: `success`})
  })
});
router.post('/historygrid', async (req, res) => {
  var code = req.body.code;
  var arr = [];
  res.setHeader("Content-Type", "application/json");
  var sql = `SELECT TOP (100) PERCENT offmonth, offmonthName, monthinis, offyear, DateOff, InComeytd, NetIncome, DateCon, code, ThaiName, 
  DATEPART(year, DateCon) AS year, DATEPART(month, DateCon) AS month
  FROM SAMCOSP.dbo.V_Historylist WHERE (code = '${code}') ORDER BY  month ,year DESC`;
  pay.query(sql, (err, records) => {
    if (records.recordset.length === 0) {
      ...
      for (let i = 0; i < records.recordset.length; i++) {
        var eNetIncome = CryptoJS.AES.decrypt(records.recordset[i]['NetIncome'], 'NetIncome').toString(CryptoJS.enc.Utf8);
        arr.push({
          offmonth: records.recordset[i]['offmonth'],
          offmonthName: records.recordset[i]['offmonthName'],
          monthinis: records.recordset[i]['monthinis'],
          offyear: records.recordset[i]['offyear'],
          DateOff: records.recordset[i]['DateOff'],
          code: records.recordset[i]['code'],
          NetIncome: eNetIncome
        })
      }
      res.json({ results: `success`, rows: arr })
    }
    // res.json({ results: `success`, rows: records.recordset })
    //res.json({ results: `success`})
  })
...
router.post('/userlogin', (req, res) => {
  ...
  let strsql = `SELECT *  FROM  SAMCOSP.dbo.DropOffUsers
    WHERE   (code = '${user}') `
...
router.get('/getsandbox', (req, res) => {

  res.setHeader("Content-Type", "application/json");
  let strsql = `SELECT * FROM  SAMCOSP.dbo.V_sandbox ORDER BY no`;
  var arow = [];
  pay.query(strsql, (err, rs) => {
    if (err) res.status(503).json({ results: `error` });
    if (rs.rowsAffected[0] === 0) {
      res.status(200).json({ results: `error` })
    } else {
      ...
      res.status(200).json({ 'results': `success`, 'requestrow': `${rs.rowsAffected[0]}`, datarows: arow })
      // res.status(200).json({ results: 'success', datarow: arow, rows: rs.rowsAffected[0] })
    }
  });
})
...
router.post('/import50tv', async (req, res) => {
  var values = req.body.dataobj;
  var userim = req.body.userim;
  var year = req.body.year;
  //console.log(userim,year)
  // console.log(values.length)
  //console.log(values[1]);
  let date_ob = new Date();
  var dayoff = ("0" + date_ob.getDate()).slice(-2) + '/' + ("0" + (date_ob.getMonth() + 1)).slice(-2) + '/' + date_ob.getFullYear();
  const table = new sql.Table('SAMCOSP.dbo.DropOff50Tv');
  table.create = true;
  table.columns.add('CreateDate', sql.NVarChar(50), { nullable: true });
  table.columns.add('CreateYear', sql.NVarChar(50), { nullable: true });
  table.columns.add('EmpImport', sql.NVarChar(50), { nullable: true });
  table.columns.add('Item', sql.NVarChar(10), { nullable: true });
  table.columns.add('Address', sql.NVarChar(300), { nullable: true });
  table.columns.add('BahtText', sql.NVarChar(200), { nullable: true });
  table.columns.add('CardID', sql.NVarChar(200), { nullable: true });
  table.columns.add('Dept', sql.NVarChar(200), { nullable: true });
  table.columns.add('EmpID', sql.NVarChar(200), { nullable: true });
  table.columns.add('FName', sql.NVarChar(200), { nullable: true });
  table.columns.add('LName', sql.NVarChar(200), { nullable: true });
  table.columns.add('Location', sql.NVarChar(100), { nullable: true });
  table.columns.add('PFYTD', sql.NVarChar(100), { nullable: true });
  table.columns.add('Pay1', sql.NVarChar(100), { nullable: true });
  table.columns.add('Pay2', sql.NVarChar(100), { nullable: true });
  table.columns.add('PayTotal', sql.NVarChar(100), { nullable: true });
  table.columns.add('Prefix', sql.NVarChar(100), { nullable: true });
  table.columns.add('ProFund', sql.NVarChar(50), { nullable: true });
  table.columns.add('Reason', sql.NVarChar(50), { nullable: true });
  table.columns.add('SSID', sql.NVarChar(100), { nullable: true });
  table.columns.add('Section', sql.NVarChar(100), { nullable: true });
  table.columns.add('Tax1', sql.NVarChar(100), { nullable: true });
  table.columns.add('Tax2', sql.NVarChar(100), { nullable: true });
  table.columns.add('TaxTotal', sql.NVarChar(100), { nullable: true });
  table.columns.add('TaxYear', sql.NVarChar(100), { nullable: true });
  table.columns.add('TaxID', sql.NVarChar(100), { nullable: true });
  for (let j = 0; j < values.length; j += 1) {
...
router.post('/postdropoff', async (req, res) => {
  ...
  var year = req.body.year;
  let date_ob = new Date();
  var checknum = `SELECT count(code) AS numrows From SAMCOSP.dbo.DropOff`
  pay.query(checknum, async (err, record) => {
    var numrow = record.recordset[0]['numrows'];
    if (numrow == 0) {
...
router.post('/postsandbox', async (req, res) => {
  ...
  //var month = req.body.month;
  //var year = req.body.year;
  const strsql = `DELETE FROM  SAMCOSP.dbo.sandbox `;
  await pay.query(strsql);
  const table = new sql.Table('SAMCOSP.dbo.sandbox');
  table.create = true;
  table.columns.add('no', sql.Int(), { nullable: true });
  // table.columns.add('offmonthName', sql.NVarChar(128), { nullable: true });
  // table.columns.add('monthinis', sql.NVarChar(128), { nullable: true });
  table.columns.add('offyear', sql.NVarChar(128), { nullable: true });
  table.columns.add('DateOff', sql.NVarChar(128), { nullable: true });
  ...
  for (let j = 0; j < values.length; j += 1) {
    ...
    var eWage = CryptoJS.AES.encrypt(values[j]['Wage'].toString(), 'Wage').toString();
    table.rows.add(no,
      date_ob.getFullYear(), dayoff, userim,
      values[j]['code'],
      values[j]['ThaiName'], values[j]['EngName'], values[j]['BankAccount'], values[j]['Advance'], values[j]['Allowance'], values[j]['AppCrem'],
      values[j]['App_Coop_SAMCO'], values[j]['BackPay'], values[j]['Blank01'], values[j]['Blank02'], values[j]['Blank03'], values[j]['Blank04'], values[j]['Blank05'],
      values[j]['Blank06'], values[j]['Blank07'], values[j]['Blank08'], values[j]['Blank09'], values[j]['Blank10'], values[j]['Card'], values[j]['Child-Edu'],
  ...
  return await request.bulk(table)
    .then(data => {
      var sql = `SELECT * FROM SAMCOSP.dbo.V_sandbox ORDER BY no`;
      var arow = [];
      pay.query(sql, (err, rs) => {
        if (err) res.status(503).json({ results: 'error' });
        for (let i = 0; i < rs.rowsAffected[0]; i++) {
        ...
        res.json({ 'results': `success`, 'requestrow': `${data.rowsAffected}`, datarows: arow })
      })
    })
    .catch(err => {
      res.json({ 'results': `error` })
...
router.post('/find50tv', (req, res) => {
  res.setHeader("Content-Type", "application/json");
  var code = req.body.code;
  let sql = `SELECT        SAMCOSP.dbo.DropOff50Tv.*
  FROM            SAMCOSP.dbo.DropOff50Tv
  WHERE (EmpID = N'${code}')`;
  pay.query(sql, (err, results) => {
    if (results.recordset.length === 0) {
      res.json({ datarow: 0, 'results': `error` })
